...
- NO changes outside `Assets/` except the 3 skill CSVs under `tiger/datafiles/skill/` (data source of truth, extended by this plan) and `FILE_MAP.md` at repo root — FILE_MAP.md MUST be updated in the same commit as every fix (user requirement: "고치면 바로바로 파일맵에 저장", 2026-07-31).
...
- [ ] 2. MeleeHitbox.cs + UseSkill type-dispatch + cooldown gate + MeleeHitbox prefab + PlayMode tests
  ...
  (d) Tests: `Assets/Tests/PlayMode/ClubGame.PlayModeTests.asmdef` (name "ClubGame.PlayModeTests", references ["Assembly-CSharp", "UnityEditor", "Unity.InputSystem"], optionalUnityReferences ["TestAssemblies"] — UnityEditor is REQUIRED: the test drives private serialized fields via `SerializedObject`; Unity.InputSystem for the Mouse device) + `Assets/Tests/PlayMode/SkillExecutionTests.cs`: build a scene programmatically that MUST include: (1) a Camera with tag "MainCamera" (UseSkill's Projectile path calls `Camera.main.ScreenToWorldPoint` — without it the test throws NRE), (2) a Mouse device — in `[UnitySetUp]` call `InputSystem.AddDevice<Mouse>()` (`using UnityEngine.InputSystem;`) and `InputSystem.RemoveDevice` in teardown (without it `Mouse.current` is null), (3) ground, enemy GameObject with Health + CircleCollider2D + a test stub `TestBubbleAffectable : MonoBehaviour, IBubbleAffectable` recording last bubble type, (4) player GameObject with PlayerController + Rigidbody2D + Collider2D + Animator; configure the private `combatSettings` (via SerializedObject `FindProperty("combatSettings")`: set `FirePoint` and `EquippedSkills` to real root assets `211_GumShot`, `201_Slash`); invoke private `UseSkill` via `typeof(PlayerController).GetMethod("UseSkill", BindingFlags.NonPublic|BindingFlags.Instance)`; reset `_skillLastUsed` via reflection between tests. Assert: (happy) projectile skill spawns an object with `Projectile` at FirePoint, enemy `Health.CurrentHealth` decreased, `TriggerCooldown` called; (mouse-aim angle is NOT asserted — identical math to working TryFire, headless mouse position is meaningless); (happy) immediate second `UseSkill(0)` within Cooldown spawns NOTHING (cooldown gate); (happy) melee skill spawns hitbox and damages an enemy exactly once over its lifetime; (failure) a skill with `ProjectilePrefab == null` spawns nothing and does NOT consume cooldown (next call after resetting time still blocked? no — assert `_skillLastUsed` unchanged and no spawn).
...
- [ ] 4. CSV schema + DataImportMenu extension (importer fields, LinkSkillPrefabs, EquipGumMasterOnPlayer)
  ...
  - `ImportSkillDataOnly()` (public static): mirrors `ImportSkillData` — the 3 `ImportSkillFile` calls + `AssetDatabase.SaveAssets(); AssetDatabase.Refresh();` — so `-executeMethod` can target skill data ONLY. DO NOT use `ImportAll` headless: it also re-imports enemy/biome data (`ImportEnemyData`/`ImportBiomeData`, DataImportMenu.cs:199-200), touching assets outside this plan's scope and tripping the F1 scope audit.
  ...
  Acceptance criteria (agent-executable): `Unity.exe -batchmode -quit -projectPath ... -executeMethod DataImportMenu.ImportSkillDataOnly -logFile -` exits with no errors in log; after running, root assets carry the table values (verify via a throwaway EditMode check or the T6 assertions); `LinkSkillPrefabs` run assigns non-null `ProjectilePrefab` to all 10 canonical assets.
  QA scenarios: happy — run ImportSkillDataOnly + LinkSkillPrefabs in batchmode, log shows no errors, evidence `.omo/evidence/task-4-skill-system-rework.txt`; failure — temporarily set `Type` to a bogus value in one CSV row, assert importer logs an error and skips the line without crashing, revert CSV.
...
- [ ] F1. Plan compliance audit — for every todo 1-6: checkbox checked, evidence file present at the documented path, commit exists with the documented message. Git log shows exactly the 6 planned commits; no file outside the documented scope (Assets/ + 3 skill CSVs + FILE_MAP.md) changed vs. the task-0 worktree snapshot. FILE_MAP.md reflects the final end-state (new files listed, skill system described, resolved issues removed from "미해결 이슈").
...
- **FILE_MAP.md must be updated IMMEDIATELY after each todo's verification passes** — never deferred to the end: add new files (MeleeHitbox.cs, test files, new prefabs), update changed component responsibilities (PlayerController UseSkill dispatch, DataImportMenu, SkillData fields), and move resolved items out of the "미해결 이슈" section. Stage FILE_MAP.md with that todo's commit so every fix lands with its docs updated.
